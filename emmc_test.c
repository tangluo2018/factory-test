...
int emmc_test(int fd, uart_msg *msg)
{
...
//while(1){
  ...
  if(msg->order == EMMC_TEST){
    ...
    total_size = system_size + cache_size + data_size + internal_size;
    
    int total = (int)(total_size + 0.5);
    ALOGI("total size %llf, total %d", total_size, total);
    send_msg(fd, EMMC_TEST, (char*)&total, sizeof(total));  
  }
//}
  ALOGI("Emmc test end");
  return 0;
